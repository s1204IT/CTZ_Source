...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = OV5675MIPI_FRONT_SENSOR_ID,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id =
				...
				read_cmos_sensor(0x300C)) + 0x0f0000;
...
{
	log_pr_debug("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable) {
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		log_pr_debug("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		log_pr_debug("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
...
UINT32 OV5675_MIPI_FRONT_RAW_SensorInit(
