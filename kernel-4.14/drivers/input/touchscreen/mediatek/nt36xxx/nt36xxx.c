 ...
 *
 * $Revision: 72882 $
 * $Date: 2020-11-24 16:33:37 +0800 (週二, 24 十一月 2020) $
 *
...
bool nvt_touch_awake(void)
{
	return bTouchIsAwake;
}

static uint8_t bFWbroken = 0;
...
info_retry:
	...
	NVT_ERR("FW_info fw_ver=%d, x_num=%d, y_num=%d, "
		"abs_x_max=%d, abs_y_max=%d, x_gang_num=%d,y_gang_num=%d max_button_num=%d!\n",
		ts->fw_ver, ts->x_num, ts->y_num,
		ts->abs_x_max, ts->abs_y_max, ts->x_gang_num, ts->y_gang_num, ts->max_button_num);
...
static int32_t nvt_extend_cmd_store(uint8_t cmd, uint8_t subcmd)
{
	uint8_t i, retry = 5;
	uint8_t buf[4] = {0};
...
#define CMD_CHARGER_PLUG_OUT 0x51
#define CMD_CHARGER_PLUG_IN  0x53
static int32_t nvt_set_charger_mode(uint8_t enable)
{
    int32_t ret = 0;

    NVT_LOG("set enable: %d ++\n", enable);

	if (enable)
		ret = nvt_cmd_store(CMD_CHARGER_PLUG_IN);
	else
		ret = nvt_cmd_store(CMD_CHARGER_PLUG_OUT);
...
static int charger_mode = -1;
static int pending_charger_mode = -1;
static bool nvt_fw_updating = false;

void set_charger_mode(int enable)
{
	int ret;
	...
	if (nvt_fw_updating) {
		pending_charger_mode = enable;
		return;
	}

	if(charger_mode != enable)
	{
		charger_mode = enable;
		ret = nvt_set_charger_mode(charger_mode);
		if(ret)
			charger_mode = -1;
	}
}

#define CMD_PALM_MODE_ON  0xB3
#define CMD_PALM_MODE_OFF 0xB4
static uint8_t nvt_palm_reject_level = 3;	/* level: 1 ~ 3 */
static int32_t nvt_set_palm_reject_mode(uint8_t level)
{
    int32_t ret = 0;
    ...
    if (level == 4)
	ret = nvt_cmd_store(CMD_PALM_MODE_OFF);
    else
        ret = nvt_extend_cmd_store(CMD_PALM_MODE_ON, level);
...
ssize_t tp_palm_reject_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	return sprintf(buf, "%d\n", nvt_palm_reject_level);
}

ssize_t tp_palm_reject_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if(count > 3)
		return -EINVAL;
	...
	nvt_palm_reject_level = buf[0] - '0';

//	printk("nvt_palm_reject_level %d\n",nvt_palm_reject_level);
	nvt_set_palm_reject_mode(nvt_palm_reject_level);

	return count;
}

DEVICE_ATTR(tp_palm_reject, 0644, tp_palm_reject_show, tp_palm_reject_store);

ssize_t tp_fw_updating_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	return sprintf(buf, "%d\n", nvt_fw_updating ? 1 : 0);
}

ssize_t tp_fw_updating_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	nvt_fw_updating = buf[0] == '1';

	if (!nvt_fw_updating && pending_charger_mode != -1 && pending_charger_mode != charger_mode) {
		set_charger_mode(pending_charger_mode);
		pending_charger_mode = -1;
	}

	return count;
}

DEVICE_ATTR(tp_fw_updating, 0644, tp_fw_updating_show, tp_fw_updating_store);
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	static uint8_t pressed_id[TOUCH_MAX_FINGER_NUM] = {0};
#endif /* MT_PROTOCOL_B */
	int32_t i = 0;
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			if(ts->swap) {
				...
				if(ts->swap_y) {
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->abs_y_max - input_y);
//					printk("id %d,x %d,input_y %d\n",input_id, ts->abs_y_max - input_y,input_x);
				}
				else {
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, input_y);
//					printk("id %d,x %d,input_y %d\n",input_id,input_y,input_x);
				}
			}
			else {
//				printk("id %d,x %d,input_y %d\n",input_id,input_x,input_y);
...
#if MT_PROTOCOL_B
	for (i = 0; i < ts->max_touch_num; i++) {
		if ((pressed_id[i] == 1) && (press_id[i] != 1)) {
			pressed_id[i] = 0;
...
//						pen_tilt_x, pen_tilt_y, pen_distance, pen_btn1, pen_btn2, pen_battery);
				...
				if(ts->swap) {
					if(ts->swap_y) {
						if (ts->wgp_stylus) {
							input_report_abs(ts->pen_input_dev, ABS_X, ts->abs_y_max * 2 - pen_y);
						}
						else {
							input_report_abs(ts->pen_input_dev, ABS_X, ts->abs_y_max - pen_y);
						}
					}
					else {
						input_report_abs(ts->pen_input_dev, ABS_X, pen_y);
					}
					input_report_abs(ts->pen_input_dev, ABS_Y, pen_x);
				}
				else{
				...
				if(ts->swap) {
					input_report_abs(ts->pen_input_dev, ABS_TILT_X, pen_tilt_y);
					input_report_abs(ts->pen_input_dev, ABS_TILT_Y, pen_tilt_x);
				}
				else {
				...
				if(pen_battery != 0)
					input_report_abs(ts->pen_input_dev, ABS_MISC, pen_battery);
			...
			if(pen_reported == 1) {
				...
				input_report_abs(ts->pen_input_dev, ABS_MISC, 0);
				input_sync(ts->pen_input_dev);
				pen_reported = 0;
			}
		}
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/****************/
	if(((ts->fw_ver & 0xf0) != 0x00) && (ts->fw_ver & 0xf0) < 0x80) {
		ts->swap = 0;
		ts->swap_y = 0;
	}
	else if((ts->fw_ver & 0x0f) < 0x05) {
		ts->swap = 1;
		ts->swap_y = 1;
	}
	else if((ts->fw_ver == 0xe5) || (ts->fw_ver == 0xd5)) {
		ts->swap = 1;
		ts->swap_y = 1;
	}
	else {
	...
	if (ts->pen_support) {
		...
		if (ts->wgp_stylus) {
			if(ts->swap) {
				input_set_abs_params(ts->pen_input_dev, ABS_X, 0, ts->abs_y_max * 2, 0, 0);
				input_set_abs_params(ts->pen_input_dev, ABS_Y, 0, ts->abs_x_max * 2, 0, 0);
			}
			else {
			...
			if(ts->swap) {
				input_set_abs_params(ts->pen_input_dev, ABS_X, 0, ts->abs_y_max, 0, 0);
				input_set_abs_params(ts->pen_input_dev, ABS_Y, 0, ts->abs_x_max, 0, 0);
			}
			else {
		...
		input_set_abs_params(ts->pen_input_dev, ABS_MISC, 0, 0xff, 0, 0);
	...
	device_create_file(&client->dev, &dev_attr_tp_palm_reject);

	device_create_file(&client->dev, &dev_attr_tp_fw_updating);
